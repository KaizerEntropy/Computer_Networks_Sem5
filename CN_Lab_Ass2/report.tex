...
\begin{titlepage}
    ...
    \includegraphics[width=0.91\textwidth]{Jadavpur_University_Logo.svg.png}\\[2 cm]

    {\huge \textbf{JADAVPUR UNIVERSITY}}\\[1.3cm]
    
    {\Large
    \textbf{Name:} Diptarko Bhattacharjee\\[0.65cm]
    \textbf{Roll:} 002410501013\\[0.65cm]
    \textbf{Class:} BCSE - UG 3 - Sem 1 - A1\\[0.65cm]
    \textbf{Subject:} Computer Networks Lab Assignment 2 Report\\
    }
...
\tikzstyle{process} = [rectangle, minimum width=3.2cm, minimum height=1.2cm, align=center, text width=3cm, draw=black, fill=orange!30]
\tikzstyle{io} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=3.2cm, minimum height=1.2cm, align=center, text width=2.5cm, draw=black, fill=blue!30]
\tikzstyle{thread} = [rectangle, rounded corners, minimum width=3.2cm, minimum height=1.2cm, align=center, text width=3cm, draw=black, fill=green!30]
\tikzstyle{arrow} = [thick,->,>=stealth]

% Sender Side
\node (in) [io] {Chunk File to Frames};
\node (setup) [process, below of=in] {Reliable Handshake (SETUP)};
\node (main) [process, below of=setup] {Main Thread:\\Transmit Data};
\node (timer) [process, below of=main] {Timeout Timers \& Retransmission};

% Background and Receiver
\node (bg) [thread, right of=main, node distance=7.5cm] {BG Thread:\\Listen for ACKs};
\node (rx) [process, below of=bg, fill=red!30] {Receiver Node};

% Connections
\draw [arrow] (in) -- (setup);
\draw [arrow] (setup) -- (main);
\draw [arrow, dashed] (setup.east) -| node[pos=0.25, above] {Spawn Thread} (bg.north);
\draw [arrow] (main) -- (timer);

% UDP Data from main to rx
\draw [arrow] (main.south east) -- node[midway, anchor=south west, align=center] {UDP\\Data} (rx.north west);

% UDP ACKs from rx to bg
\draw [arrow] (rx.north) -- node[midway, anchor=west] {UDP ACKs} (bg.south);

% Update Map from bg to main
\draw [arrow] (bg.west) -- node[midway, above, align=center, yshift=1mm] {Update Map\\\& Window Base} (main.east);

% Timer triggers slide window
\draw [arrow] (timer.west) -- ++(-1,0) |- node[pos=0.25, left, align=center] {Slide\\Window} (main.west);
...
To empirically compare the three ARQ mechanisms, we streamed a test payload representing exactly 7 datagram chunks through the UDP sockets on the local loopback interface. We simulated a violently noisy network by artificially injecting a $20\%$ probability of Bit Error and a $10\%$ probability of Packet Loss. 

Furthermore, to rigorously test the protocol state machines against severe real-world latency scenarios, a probabilistic artificial transmission delay module was implemented within the `channel\_transmit` function. Just like error and packet loss rates, the user inputs a custom probability parameter for delay. If triggered based on this probability, a physical \texttt{std::this\_thread::sleep\_for(20ms)} delay is injected prior to frame dispatch. This successfully emulated a network where the Propagation Delay vastly exceeds the Sender's hardcoded socket timeout (which was aggressively clamped at 10ms). This "Premature Timeout" condition (where the Timeout Timer $<$ RTT) proved catastrophic for standard flow control. It artificially induced massive redundant retransmissions, forcing the Sender to falsely assume the packet was lost and resend it before the receiver's valid ACK could traverse the high-latency wire.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{efficiency_comparison.png}
    \caption{Theoretical Throughput Efficiency ($\eta$) vs. Packet Error Probability ($p$)}
    \label{fig:efficiency}
\end{figure}

To theoretically map the constraints that we will observe in the upcoming empirical dry run, a Python script was written to plot the throughput efficiency ($\eta$) of all three algorithms as the error probability ($p$) scaled from 0.0 to 0.5. As shown in Figure \ref{fig:efficiency}, while \textbf{Stop-and-Wait} suffers from a flat, consistently terrible baseline efficiency, \textbf{Go-Back-N's} efficiency violently plummets the moment $p > 0.1$ due to discarding entire windows. In stark contrast, the \textbf{Selective Repeat} curve maintains a dominant, near-linear degradation, proving it is the only mathematically viable ARQ protocol for the high-loss environment we are about to simulate.
...
\begin{verbatim}
--------------------------------------------------------------------------------------
Detailed Event Trace (Stop-and-Wait ARQ with Probabilistic 20ms Latency)
--------------------------------------------------------------------------------------
[00:00:00] [Sender] Preparing Frame 1 (Seq: 0). FCS Computed.
[00:00:01] [Sender] Transmitting Frame 1. 20ms probabilistic delay applied. 10ms Timer starts.
[00:00:11] [Sender] *** TIMEOUT *** 10ms threshold breached for Frame 1!
[00:00:12] [Sender] Premature Retransmission of Frame 1! Restarting 10ms timer.
[00:00:21] [Recv]   Frame 1 (Original) arrives. FCS valid. Seq 0 expected. 
[00:00:22] [Recv]   Extracting Payload 1. Generating ACK for Seq 1 (Next Expected).
[00:00:23] [Recv]   Transmitting ACK 1 over UDP.
[00:00:25] [Sender] Receives ACK 1. Matches expected. Timer cancelled. 

[00:00:26] [Sender] Preparing Frame 2 (Seq: 1). FCS Computed.
[00:00:27] [Sender] Transmitting Frame 2. No delay applied (prob_delay missed). 10ms Timer starts.
[00:00:28] [Network] *** INTERFERENCE *** Bit 56 flipped in Frame 2.
[00:00:30] [Recv]   Frame 2 arrives. FCS MISMATCH! Hash failed.
[00:00:31] [Recv]   Silently discarding corrupted Frame 2. No ACK generated.
[00:00:37] [Sender] *** TIMEOUT *** 10ms threshold breached for Frame 2!
[00:00:38] [Sender] Normal Retransmission of Frame 2! Restarting 10ms timer.
[00:00:42] [Recv]   Frame 2 (Retransmission) arrives. FCS valid. Seq 1 expected.
[00:00:43] [Recv]   Extracting Payload 2. Generating ACK for Seq 0.
[00:00:45] [Sender] Receives ACK 0. Matches expected. Timer cancelled.
--------------------------------------------------------------------------------------
\end{verbatim}
\end{scriptsize}
...
\begin{verbatim}
--------------------------------------------------------------------------------------
Detailed Event Trace (Go-Back-N, N=4, Probabilistic 20ms Latency)
--------------------------------------------------------------------------------------
[00:00:00] [Sender Main] Transmitting Frame 1 (Seq: 0). Base=0. No delay applied.
[00:00:01] [Sender Main] Transmitting Frame 2 (Seq: 1). Base=0. 20ms probabilistic delay starts.
[00:00:02] [Sender Main] Transmitting Frame 3 (Seq: 2). Base=0. No delay applied.
[00:00:03] [Sender Main] Transmitting Frame 4 (Seq: 3). Window FULL. No delay applied.
...
[00:00:07] [Sender BG]   Received ACK 1! Base_seq slides to 1. Window opens!
[00:00:08] [Sender Main] Transmitting Frame 5 (Seq: 4). Base=1. No delay applied.

[00:00:10] [Recv]        Frame 3 (Original) arrives. Valid FCS. Seq 2 != Expected 1! 
[00:00:11] [Recv]        OUT-OF-ORDER. Discarding perfectly valid Frame 3.
[00:00:12] [Recv]        Frame 4 (Original) arrives. Valid FCS. Seq 3 != Expected 1!
[00:00:13] [Recv]        OUT-OF-ORDER. Discarding perfectly valid Frame 4.
[00:00:14] [Recv]        Frame 5 (Original) arrives. Valid FCS. Seq 4 != Expected 1!
[00:00:15] [Recv]        OUT-OF-ORDER. Discarding perfectly valid Frame 5.

[00:00:17] [Sender Main] *** TIMEOUT *** Base timer (Frame 2) breached 10ms threshold!
[00:00:18] [Sender Main] Premature Avalanche! Re-Transmitting Frames 2, 3, 4, 5.

[00:00:25] [Recv]        Frame 2 (Original, Delayed) arrives. Valid! Seq 1 matches expected.
[00:00:26] [Recv]        Sending Cumulative ACK 2. Expected_seq updated to 2.

[00:00:30] [Network]     *** INTERFERENCE *** ACK 2 completely dropped!
[00:00:32] [Recv]        Frame 2 (Retransmission) arrives. Redundant! Expected 2.
[00:00:33] [Recv]        Frame 3 (Retransmission) arrives. Valid! Seq 2 matches expected. 
[00:00:34] [Recv]        Sending Cumulative ACK 3. Expected_seq updated to 3.
[00:00:35] [Recv]        Frame 4 (Retransmission) arrives. Valid! Seq 3 matches expected. 
[00:00:36] [Recv]        Sending Cumulative ACK 4. Expected_seq updated to 4.

[00:00:40] [Sender BG]   Received ACK 4! (Cumulative). 
[00:00:41] [Sender BG]   ACK 4 mathematically implies Frame 2 and Frame 3 were received despite ACK 2 loss!
[00:00:42] [Sender BG]   Base_seq violently slides from 1 to 4! Timer resets.
--------------------------------------------------------------------------------------
\end{verbatim}
\end{scriptsize}
...
\begin{verbatim}
--------------------------------------------------------------------------------------
Detailed Event Trace (Selective Repeat, N=4, Probabilistic 20ms Latency)
--------------------------------------------------------------------------------------
[00:00:00] [Sender Main] Transmitting Frame 1 (Seq: 0). Base=0. No delay applied.
[00:00:01] [Sender Main] Transmitting Frame 2 (Seq: 1). 20ms probabilistic delay starts.
[00:00:02] [Sender Main] Transmitting Frame 3 (Seq: 2). No delay applied.
[00:00:03] [Sender Main] Transmitting Frame 4 (Seq: 3). Window FULL. No delay applied.

[00:00:05] [Recv]        Frame 1 (Original) arrives. Seq 0 expected. Pushed to final array.
[00:00:06] [Recv]        Sending Independent ACK for Seq 0.
[00:00:07] [Sender BG]   Received ACK for Seq 0. Dictionary marked TRUE. Base_seq slides to 1.
[00:00:08] [Sender Main] Window opened. Transmitting Frame 5 (Seq: 4). No delay.

[00:00:10] [Recv]        Frame 3 (Original) arrives. Valid FCS. Seq 2 != Expected 1.
[00:00:11] [Recv]        Seq 2 is inside (1 + 4). Allocating map buffer!
[00:00:12] [Recv]        Caching Payload 3. Sending Independent ACK for Seq 2.
[00:00:13] [Recv]        Frame 4 (Original) arrives. Valid FCS.
[00:00:14] [Recv]        Seq 3 is inside (1 + 4). Allocating map buffer!
[00:00:15] [Recv]        Caching Payload 4. Sending Independent ACK for Seq 3.
[00:00:16] [Recv]        Frame 5 (Original) arrives. Valid FCS.
[00:00:17] [Recv]        Seq 4 is inside (1 + 4). Allocating map buffer!
[00:00:18] [Recv]        Caching Payload 5. Sending Independent ACK for Seq 4.

[00:00:19] [Sender Main] *** TIMEOUT *** Independent timer for Frame 2 breached 10ms threshold!
[00:00:20] [Sender Main] Surgically Re-Transmitting ONLY Frame 2.

[00:00:25] [Recv]        Frame 2 (Original, Delayed) arrives. Valid! Seq 1 expected.
[00:00:26] [Recv]        Pushing Payload 2 to final array. Expected_seq = 2.
[00:00:27] [Recv]        Checking map: Seq 2 is buffered! Pushing Payload 3. Expected = 3.
[00:00:28] [Recv]        Checking map: Seq 3 is buffered! Pushing Payload 4. Expected = 4.
[00:00:29] [Recv]        Checking map: Seq 4 is buffered! Pushing Payload 5. Expected = 5.
[00:00:30] [Recv]        Contiguous block fully flushed! Sending Independent ACK for Seq 1.

[00:00:33] [Recv]        Frame 2 (Retransmission) arrives. Redundant! Discarding.
[00:00:35] [Sender BG]   Received ACK for Seq 1. Dictionary marked TRUE.
[00:00:36] [Sender BG]   Contiguous dictionary crawl: 1 is True, 2 is True, 3 is True, 4 is True.
[00:00:37] [Sender BG]   Base_seq violently slides from 1 to 5 simultaneously! Window wide open!
--------------------------------------------------------------------------------------
\end{verbatim}
\end{scriptsize}
...
When the Sender application completes transmission in a noisy, high-latency environment (10\% Loss, 20\% Error, 20ms Transmission Delay), our `utils.cpp` module renders a beautiful statistical dashboard summarizing the devastation. Below is the literal output captured from the execution:
...
============================================================
 ...
 Simulated Latency  : 20 ms (Timeout = 10 ms)
------------------------------------------------------------
 Total Data Frames  : 65
 Total Transmitted  : 140 (Includes premature retransmissions)
 Total Timeouts     : 75
 Total Time Elapsed : 4890 ms
------------------------------------------------------------
 ...
 - Avg RTT          : 24.5 ms
------------------------------------------------------------
 Throughput Efficiency (Unique/Total Transmissions) : 46.42 %
============================================================
...
- The 20ms Latency vastly overpowered the 10ms socket timeout, causing 75 premature timeouts!
- Efficiency drops proportionally to both the error rates AND the latency-induced Avalanche.
============================================================
\end{verbatim}

\textbf{Dashboard Deconstruction:} The visualizer proves that out of the 140 raw transmissions fired by the sender, only 65 were unique, valid payloads. A staggering 75 transmissions were purely redundant waste. Unlike the previous test, this waste was not primarily caused by FCS corruption; it was caused by the \texttt{sleep\_for(20ms)} delay severely punishing the aggressive 10ms socket timeout, inducing premature timeouts before the receiver could even process the data. The RTT analysis accurately profiles this, showing a minimum RTT clamped strictly above the 20ms physical barrier, averaging 24.5ms of total propagation delay.
